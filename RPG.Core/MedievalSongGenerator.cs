...
public class MedievalProceduralAudio
{
	...
	private void GenerateHarmonyLayer(float[] buffer, int durationSeconds)
	{
		...
		while (currentSample < totalSamples)
		{
			...
			int[] harmonyIntervals = { 4, 7 }; // Major 3rd, Perfect 5th
	...
	private void AddNote(float[] buffer, int startSample, int length, float freq, float velocity)
	{
		...
		// ADSR envelope parameters (samples)
		int attack = (int)(0.05f * sampleRate);
		int decay = (int)(0.1f * sampleRate);
		float sustainLevel = 0.7f;
		int release = (int)(0.15f * sampleRate);
		...
		for (int i = startSample; i < endSample; i++)
		{
			...
			if (notePos < attack)
				envelope = (float)notePos / attack;
			else if (notePos < attack + decay)
				envelope = 1f - (1f - sustainLevel) * ((notePos - attack) / (float)decay);
			else if (notePos < length - release)
				envelope = sustainLevel;
			else if (notePos < length)
				envelope = sustainLevel * (1f - (notePos - (length - release)) / (float)release);
			else
				envelope = 0f;
			...
			sampleValue *= envelope * velocity;

			buffer[i] += sampleValue;
		}
	}

	private float MidiNoteToFrequency(float midiNote)
	{
